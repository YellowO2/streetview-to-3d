...
Turns downloaded panos into an actual 3d point cloud, via real pairwise DA3 tests between panos.
    ...
    - 3.2 Join (join_segments.py: `join_segments`, `bridge_pieces`) -- in the same GPU call as 3.1
    Segments from 3.1 are each in their own arbitrary DA3-local frame (nothing ties them together yet). Join tries to stitch them: for every pair of segments, pick node pairs within `BRIDGE_MAX_DIST_M` (same date first, then closest) and run a real DA3 test between them; success chains a rigid-align onto the existing piece and merges the two segments into one. Unlike the walk, a bridge may join two different dates. No GPS placement is used -- only real DA3-confirmed connections merge anything.
    Output: **pieces** -- `[(points, colors, metadata), ...]`, one per still-separate result. metadata is `{node_key: {lat, lon, date, position, rotation, n_views_kept, n_views_total, links}}` -- `links` being this node's DA3-confirmed neighbours and the keep counts from each joint test. More than one piece means bridging genuinely couldn't connect everything, which is not an error.
...
A piece arrives internally consistent but individually placed: its own DA3 frame has an arbitrary origin, rotation and scale, and nothing ties it to the ground or to its neighbours. `postprocess.pipeline.process` is the whole stage as one call.
    ...
    - 4.1 Fit to GPS (gps_fit/load_pieces.py). A piece is the nodes sharing a DA3 frame -- a connected component of the scene's edges, derived on demand rather than stored, since joining a node to a piece is exactly what puts it in that frame. Each node carries both its DA3 position and its pano's real lat/lon, so a 2D fit (rotation and translation) puts the piece on the map. Height is NOT fitted here -- the fit is top-down only. Scale is fitted once per scene, never per piece: the median over pieces whose cameras span at least 8 m, falling back to `config.DA3_UNITS_TO_METRES` (see `load_pieces.scene_scale`). A single-node piece is turned by its own camera heading.
    - 4.2 Road alignment (road_align/node_center_to_road_line.py). Road centre lines are built from the scene's own street graph (corridors.py chains dots into roads). Each piece slides sideways onto its road's line. A piece with more than two nodes may only translate -- GPS already measured its heading; a 2-node piece may also rotate. Then cross_road.py compares pieces on the same road by the ground's height profile across it and solves every sideways shift at once.
    - 4.3 Height (road_align/ground_elevation.py). Nothing in a reconstruction says how high a piece sits, so the ground itself supplies the datum. Every pano lookup returns an elevation, so step 2.1 already recorded one per dot and the scene's graph carries them -- no second round of network calls, and coverage over the whole area rather than only the dots that reconstructed. They are metres above sea level (Y-UP) and are negated on the way in (see the README's coordinate convention). A smooth surface is fitted through them and each piece gets one height offset and one tilt onto it.
