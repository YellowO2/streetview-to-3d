...
@GPU_WINDOWED
def run_pathfind_reconstruction_gpu(nodes, edges, start_lat, start_lon, end_lat, end_lon, step_degrees=20, date_order=None):
    pipeline = get_pipeline()
    return pipeline.run_pathfind_reconstruction(
        nodes, edges, start_lat, start_lon, end_lat, end_lon, step_degrees=step_degrees, date_order=date_order
    )
