...
A whole campus is too large for one GPU call, so it's split into connected chunks (`map_selection.candidates.split_into_chunks`, ~20 real nodes each) and run one at a time, incrementally bridged onto whatever's already merged so far -- not generated all at once then joined once at the end.

    - 4.1 `cli_run_chunk` -- Prepare + Run for ONE chunk (its own `@spaces.GPU` call). Saves this chunk's own raw, un-bridged segments to a dataset repo (`CLI_RAW_PREFIX/<chunk_id>`) immediately, independent of whatever bridging does with them next -- so a bug in bridging never requires re-running this (expensive) step, only re-running the bridge against the same saved data.
